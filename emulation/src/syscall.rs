...
unsafe fn get_allow_slices(sock: &UnixDatagram) {
    loop {
        ...
        let slice: &mut [u8] =
            std::slice::from_raw_parts_mut(preamble.address as *mut u8, preamble.length);
...
unsafe fn supply_allow_slices(sock_rx: &UnixDatagram, sock_tx: &UnixDatagram) {
    loop {
        ...
        if len != preamble.length {
            panic!(
                "Kernel recieved slice mismatch, expected {}, but sent {}",
                preamble.length, len
            );
        }
...
impl Syscall {
    ...
    pub fn invoke(self) -> isize {
        let as_bytes = unsafe { transmute::<Syscall, [u8; size_of::<Syscall>()]>(self) };
        ...
        unsafe {
            supply_allow_slices(kernel_rx, kernel_tx);
        }
        ...
        if len != RETURN_SIZE {
            // TODO generate error
            println!(
                "Weird syscall return size {}, should be {}",
                len, RETURN_SIZE
            );
        }

        unsafe {
            get_allow_slices(kernel_rx);
        }

        let kernel_return = unsafe { transmute::<[u8; RETURN_SIZE], KernelReturn>(buf) };
